...
@pytest.mark.asyncio
@pytest.mark.everest_core_config('probe-system.yaml')
async def test_network_interfaces_var(everest_core: EverestCore, async_api_mqtt_handler: AsyncApiMqttHandler, probe_module: ProbeModule):

    mqtt_prefix = everest_core.mqtt_external_prefix
    topic = f"{mqtt_prefix}everest_api/1/system/system_api/m2e/network_interfaces"

    queue = probe_module.subscribe_variable_to_queue('system', 'network_interfaces')

    interfaces_payload = [
        {
            "name": "eth0",
            "up": True,
            "interface_class": "Wired0",
            "ip_addresses": ["192.168.1.10"]
        }
    ]
    result = await _publish_until_received(async_api_mqtt_handler, queue, topic, interfaces_payload)
    assert result == interfaces_payload


@pytest.mark.asyncio
@pytest.mark.everest_core_config('probe-system.yaml')
async def test_network_interfaces_payload_shapes(everest_core: EverestCore, async_api_mqtt_handler: AsyncApiMqttHandler, probe_module: ProbeModule):
    ...
    # empty array is a valid array -> surfaces as an empty list
    result = await _publish_until_received(async_api_mqtt_handler, queue, topic, [])
    assert result == []

    # multi-element array surfaces in order (catches loop off-by-one)
    multi = [
        {"name": "eth0", "up": True},
        {"name": "wwan0", "up": False, "interface_class": "Wireless0"},
    ]
    result = await _publish_until_received(async_api_mqtt_handler, queue, topic, multi)
    assert result == multi

    # non-array payload fails closed -> nothing is published on the var
    _drain_queue(queue)
    await async_api_mqtt_handler.publish(topic, json.dumps({"eth0": {"up": True}}))
    loop = asyncio.get_event_loop()
    with pytest.raises(Empty):
        await loop.run_in_executor(None, lambda: queue.get(timeout=2))
