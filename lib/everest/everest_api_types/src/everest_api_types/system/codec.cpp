...
std::string serialize(NetworkInterfaceStatus const& val) noexcept {
    return utilities::dump_json(val);
}
...
std::ostream& operator<<(std::ostream& os, NetworkInterfaceStatus const& val) {
    os << serialize(val);
    return os;
}
...
template <> NetworkInterfaceStatus deserialize(std::string_view val) {
    return utilities::parse_json<NetworkInterfaceStatus>(val);
}
