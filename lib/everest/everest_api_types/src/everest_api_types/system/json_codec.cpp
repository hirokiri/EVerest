...
void to_json(json& j, const VPN& k) noexcept {
    j = json{
        {"server", k.server}, {"user", k.user}, {"password", k.password}, {"key", k.key}, {"type", k.type},
    };
    if (k.group) {
...
void to_json(json& j, const NetworkInterfaceStatus& k) noexcept {
    j = json{
        {"name", k.name},
        {"up", k.up},
    };
    if (k.interface_class) {
        j["interface_class"] = k.interface_class.value();
    }
    if (k.ip_addresses) {
        j["ip_addresses"] = k.ip_addresses.value();
    }
}
void from_json(const json& j, NetworkInterfaceStatus& k) {
    k.name = j.at("name");
    k.up = j.at("up");

    if (j.contains("interface_class")) {
        k.interface_class.emplace(j.at("interface_class"));
    }
    if (j.contains("ip_addresses")) {
        k.ip_addresses.emplace(j.at("ip_addresses").get<std::vector<std::string>>());
    }
}
