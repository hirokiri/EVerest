...
NetworkInterfaceStatus_Internal to_internal_api(NetworkInterfaceStatus_External const& val) {
    NetworkInterfaceStatus_Internal result;
    result.name = val.name;
    result.up = val.up;
    if (val.interface_class.has_value()) {
        result.interface_class.emplace(to_internal_api(val.interface_class.value()));
    }
    result.ip_addresses = val.ip_addresses;
    return result;
}
NetworkInterfaceStatus_External to_external_api(NetworkInterfaceStatus_Internal const& val) {
    NetworkInterfaceStatus_External result;
    result.name = val.name;
    result.up = val.up;
    if (val.interface_class.has_value()) {
        result.interface_class.emplace(to_external_api(val.interface_class.value()));
    }
    result.ip_addresses = val.ip_addresses;
    return result;
}
